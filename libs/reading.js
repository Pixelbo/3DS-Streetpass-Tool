...
exports.readHex = function (data_or_path, offset_, size, loaded) {
    ...
    if (!loaded) {
        var buffer = fs.readFileSync(data_or_path, { encoding: "hex" });
    } else {
        var buffer = data_or_path;
    }
...
exports.hexToUTF = function (HexString) {
    ...
    HexString.match(/.{4}/g).forEach((code) => {
        buffer.push(String.fromCodePoint(parseInt(code.replace("00", ""), 16)));
    });
    buffer = buffer.join("");
...
exports.reverse_endian = function (num) {
    var reversed = num.match(/../g).reverse().join("");
    return reversed;
};
